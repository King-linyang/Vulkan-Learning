...
void MyVulkanFixedFuncs::createTriangle(MyVulkanSwapChain myVulkanSwapChain, VkDevice device,
    ...
    //视口和裁剪阶段 视口基本上描述了输出将被渲染到的帧缓冲区区域
    VkViewport viewport{};
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = (float) myVulkanSwapChain.getSwapChainExtent().width;
    viewport.height = (float) myVulkanSwapChain.getSwapChainExtent().height;
